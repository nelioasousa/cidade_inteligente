...
import time
import json
from messages_pb2 import Address, GatewayLocation, JoinRequest, JoinReply


GATEWAY_IP = socket.gethostbyname(socket.gethostname())
GATEWAY_CLIENT_PORT = 5000
GATEWAY_DEVICES_PORT = 54321
...
def multicast_gateway_location(interval_sec=5):
    ...
    msg = GatewayLocation(Address(GATEWAY_IP, GATEWAY_DEVICES_PORT))
    while True:
...
def join_handler(sock):
    with sock:
        try:
            ...
            sock.send(JoinReply(REPORT_INTERVAL).SerializeToString())
        finally:
...
def join_listener():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.bind(('', GATEWAY_DEVICES_PORT))
        sock.listen()
        while True:
            conn, _ = sock.accept()
